	...
	class CustomView extends htmlElementType implements BaseComponent<T>, CustomElement {
		...
		doBlockCallback = (): void => {
			if (isDoCheck(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.doCheck, this._model);
			}
		};
		...
		attributeChangedCallback(name: string, oldValue: string, newValue: string) {
			...
			if (isOnChanges(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.onChanges, this._model);
			}
		...
		connectedCallback() {
			...
			if (isOnInit(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.onInit, this._model);
			}
			if (isDoCheck(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.doCheck, this._model);
			}
			if (isAfterContentInit(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.afterContentInit, this._model);
			}
			if (isAfterContentChecked(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.afterContentChecked, this._model);
			}
			...
			if (isAfterViewInit(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.afterViewInit, this._model);
			}
			if (isAfterViewChecked(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.afterViewChecked, this._model);
			}
			this.doBlockCallback = () => {
				if (isDoCheck(this._model)) {
					this._auroraZone.runAsScopeTask(this._modelScope, this._model.doCheck, this._model);
		...
		disconnectedCallback() {
			...
			if (isOnDestroy(this._model)) {
				this._auroraZone.runAsScopeTask(this._modelScope, this._model.onDestroy, this._model);
			}
			this.subscriptions.forEach(sub => sub.unsubscribe());
			this.subscriptions.splice(0, this.subscriptions.length);
		}
